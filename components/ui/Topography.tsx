...
// Ported from reactbits.dev's Topography background (https://reactbits.dev/backgrounds/topography) —
// an animated topographic-map effect: contour lines traced as a distance
// field between two bezier-warped curves, colored by "elevation" and
// continuously re-warped over time. Retinted here to the app's own pine/clay
// palette (see app/globals.css) instead of the original violet/pink demo
// colors, with the same interaction (mouse creates a local bump in the
// field) and grain/glow finishing passes intact.
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      try {
        ...
        // Already removed (e.g. StrictMode double-invoke) — nothing to clean up.
      }
